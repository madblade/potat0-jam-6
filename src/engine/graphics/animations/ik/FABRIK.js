...
)
{
    ...
    for (let loop = 0; loop < chainLength; ++loop)
    {
        ...
        if (loop !== 0)
        {
            ...
            const id = link.id;
            if (id !== loop) throw Error('Please specify all links in the constraints array.');
            const limitation2 = link.limitation;
            const rotationMin2 = link.rotationMin;
            const rotationMax2 = link.rotationMax;
            if (!activateConstraints) jointType = JointType.NONE;
            else if (limitation2) jointType = JointType.LOCAL_HINGE;
            else if (rotationMin2 || rotationMax2) jointType = JointType.BALL;
            ...
            if (jointType === JointType.BALL)
            {
                ...
                this.applyBallConstraint(thisBoneInnerToOuterUV, previousBoneInnerToOuterUV,
                    correctionQuaternion, rotationMin2, rotationMax2);
            }
            ...
            else if (jointType === JointType.LOCAL_HINGE)
            {
                ...
                const prevBone = proxy[loop - 1];
                previousBoneInnerToOuterUV
                    .copy(thisBonePosition)
                    .addScaledVector(prevBone, -1)
                    .normalize();
                ...
                relativeHingeRotationAxis.copy(limitation2).applyQuaternion(localTransform);
                ...
                if (rotationMin2 !== undefined || rotationMax2 !== undefined)
                {
